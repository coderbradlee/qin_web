...
<% 	set ris=server.createobject("adodb.recordset")
	...
	sqli="select * from jiedai_yinhang where e_classid<>'' "
	else
    sqli="select * from jiedai_yinhang where id="&request.QueryString("a")
	end if
...
<!-- .nleft -->
  ...
  <li><a href="About.asp?a=<%=res("id")%>" <%if aid=int(res("id")) or i=0 then  response.Write"class=""focus""" end if%> ><%=res("e_classid")%></a></li>
