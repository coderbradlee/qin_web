...
<!-- .nleft -->
  ...
  <li><a href="partner.asp?a=<%=res("id")%>"  ><%=res("e_classid")%></a></li>
          ...
          <%
					  ...
					  set res=nothing
					  %>
	<li><A href="partner_list.asp" class="focus" >more....</A></li>
 ...
 <!-- .ncenter -->
 <div class="ncenter">
	<ul class="nc_title">Home > Partner  <span> > <% =a_title %></span></ul>
   	<%call banner(204)%>
    <ul class="nbody">
...
mypage.getconn=conn
mypage.getsql="select * from partner_fuwu where  classid<>'' order by  flag asc"
mypage.pagesize=15
set rs=mypage.getrs()
...
%>
		...
		<li><a href="partner_show.asp?id=<% =rs("id") %>" ><% =rs("e_classid") %></a><span>[<% =FormatDate(rs("addtime"),4) %>]</span></li>
