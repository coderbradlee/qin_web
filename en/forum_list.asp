...
<!-- .nleft -->
  ...
  <li><a href="forum.asp?a=<%=res("id")%>"  ><%=res("e_classid")%></a></li>
          ...
          <%
					  ...
					  set res=nothing
					  %>
	<li><A href="forum_list.asp" class="focus" >more....</A></li>
...
mypage.getconn=conn
mypage.getsql="select * from forum_fuwu where  classid<>'' order by  flag asc"
mypage.pagesize=15
set rs=mypage.getrs()
...
%>
		...
		<li><a href="forum_show.asp?id=<% =rs("id") %>" ><% =rs("e_classid") %></a><span>[<% =FormatDate(rs("addtime"),4) %>]</span></li>
