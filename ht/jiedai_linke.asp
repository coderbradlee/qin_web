...
if trim(request.form("submit"))="添 加" then
	...
	title=trim(request.form("image"))
	url=trim(request.form("url"))
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_Linke"
...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from jiedai_Linke where flag<"&OrderBy&" order by flag desc" 
		set rs=server.createobject("adodb.recordset") 
		...
		'修改前一条记录的id 
		sql2="select flag from jiedai_Linke where id="&before_id '查询前一条记录 
		...
		sql3="select flag from jiedai_Linke where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from jiedai_Linke where flag>"&OrderBy&" order by flag asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select flag from jiedai_Linke where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select flag from jiedai_Linke where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
  ...
  <tr>
    <td><table width="100%" height="174" border="0" cellpadding="0" cellspacing="0">
      <form name="form1" id="form2" method="post" action="?action=list" onSubmit="return check_add()">
        <tr>
          <td width="305" align="right">图片地址：<font color="#ff0000">[必填]</font></td>
          <td style="padding-left:6px;"><input name="image" type="text" class="STYLE44line1" id="image" size="30" style="cursor:hand">
              <font color="#ff0000">*
              <iframe src="tongyongsc.asp" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe>
              </font></td>
        </tr>
        <tr>
...
set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_Linke order by flag asc"
...
for i=1 to rs.pagesize
 ...
 %>
      <table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
        <tr>
          <td width="338" align="left" style="padding-left:6px"><%=i%>-<a href="<%=rs("url")%>" target=_blank><img src="../uploadfile/<%=rs("title")%>" width="88" height="31" border="0" align="absmiddle"></a></td>
          <td>&nbsp;<%= rs("url")  %></td>
...
if trim(request.querystring("action"))="del" then
		...
		set rs=server.createobject("adodb.recordset")
		sql="select * from jiedai_Linke where id="&id
		rs.open sql,conn,1,3
		rs.delete
		rs.update
		rs.requery
...
<br>
<table width="100%" height="153" border="0" cellpadding="0" cellspacing="0" bordercolor="#215dc6">
  <form name="form1" id="form1" method="post" action="?action=edit&id=<%= trim(request.querystring("id")) %>" onSubmit="return check_edit()">
    <tr> 
      ...
      <td style="padding-left:8px"> <input name="image" type="text" class="STYLE44line1" id="image"  value="<%= rs("title") %>" size="30"> 
        <font color="#ff0000">*
        <iframe src="tongyongsc.asp" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe>
        </font></td>
    </tr>
