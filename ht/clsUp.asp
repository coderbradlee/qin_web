...
Public Sub GetData (MaxSize)
	...
	if isGetData_=false then 
		...
		If Request.TotalBytes < 1 Then	'如果没有数据上传
			isErr_ = 1
			ErrMessage_="<a href=javascript:history.go(-1)>没有数据上传</a>"
			Exit Sub
		End If
		If MaxSize > 0 Then '如果限制大小
			If Request.TotalBytes > MaxSize Then
			isErr_ = 2	'如果上传的数据超出限制大小
			ErrMessage_="<a href=javascript:history.go(-1)>上传的数据超出限制大小</a>"
			Exit Sub
			End If
		End If
...
Private Function SaveToFileEx(Item,Path,Over)
	...
	if Over then
		if isAllowExt(GetFileExt(tmpPath)) then
			...
			Else
			isErr_=3
			ErrMessage_="<a href=javascript:history.go(-1)>该后缀名的文件不允许上传!</a>"
		End if
	Else
