...
if trim(request.querystring("action"))="del" then
	...
	sql="select * from jiedai_Fangkuan where id="&id
...
<%
	...
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_Fangkuan where 1=1 "
	...
	if keywords<>"" then
	sql=sql+" and user_name like '%"&keywords&"%' or Fangkuan_title like '%"&keywords&"%'"
	end if
...
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc" class="zw">
  <tr>
    <td width="143" align="center" class="line"><%=rs("fangkuan_bianhao")%></td>
    <td class="line">&nbsp;<a href="../Jiedai_Jiekuan_Show.asp?jid-jiedai=<%=rs("id")%>&uid=<%=left(rs("user_uid"),4)%>-<%=rs("user_uid")%>" class="zwe" target="_blank"><%= rs("Fangkuan_Title") %></a></td>
    <td width="68" align="center" class="line"><a href="?user_ename=<%=rs("user_name")%>"><%if rs("user_name")="jiedai_niming" then response.write"匿名" else response.write rs("user_name")%></a></td>
    <td width="260" align="center" class="line"><%=rs("addtime")%></td>
    ...
    <td width="50" align="center" class="line"><a href="Edit_Fangkuan.asp?jid=<%= rs("id") %>&page=<%=page%>&keywords=<%=keywords%>&user_ename=<%=user_ename%>" style="font-size:12px; color:#666666"><img src="images/Edit.gif" alt="修改信息" width="12" height="12" border="0" /></a></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
