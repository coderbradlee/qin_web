...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from zhibenhui_fuwu where flag<"&OrderBy&" order by flag desc" 
		...
		'修改前一条记录的id 
		sql2="select flag from zhibenhui_fuwu where id="&before_id '查询前一条记录 
		...
		sql3="select flag from zhibenhui_fuwu where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from zhibenhui_fuwu where flag>"&OrderBy&" order by flag asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select flag from zhibenhui_fuwu where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select flag from zhibenhui_fuwu where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
...
set rs=server.createobject("adodb.recordset")
sql="select * from zhibenhui_fuwu order by flag asc"
...
<% if trim(request.querystring("action"))="add" then
	if trim(request.form("add"))="add" then
		...
		set rs=server.createobject("adodb.recordset")
		sql="select * from zhibenhui_fuwu"
		rs.open sql,conn,1,3
		rs.addnew
