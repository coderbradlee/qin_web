...
upfile.NoAllowExt="tmp;asa;inc;asp;php;js;vb;vbs;html;htm;aspx;sql;txt;cer;"
...
if upfile.isErr then  '如果出错
	...
	if upfile.iserr then 
		...
		vff=Trim(Request.QueryString("ffs"))
		vid=Trim(Request.QueryString("id"))
		
		if  vid<>"" then
		Response.Write "上传成功 , <A href='jiedai_up.asp?ffs="&vff&"&id="&vid&"' >重新上传</a>"		
	'response.write"<script>parent."&vff&"."&vid&".value='"& session("file") &"';<//script>"
	response.write"<script>parent.document.getElementById('"&vid&"').value='"& session("file") &"';</script>"
		
		else
		Response.Write "上传成功 , <A href='jiedai_up.asp' >重新上传</a>"		
		response.write"<script>parent.form1.image.value='"& session("file") &"';</script>"
		end if
