...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from jiedai_ziyuan where flag<"&OrderBy&" order by flag desc" 
		...
		'修改前一条记录的id 
		sql2="select flag from jiedai_ziyuan where id="&before_id '查询前一条记录 
		...
		sql3="select flag from jiedai_ziyuan where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from jiedai_ziyuan where flag>"&OrderBy&" order by flag asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select flag from jiedai_ziyuan where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select flag from jiedai_ziyuan where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
...
set rs=server.createobject("adodb.recordset")
sql="select * from jiedai_ziyuan order by flag asc"
...
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc" bgcolor="#D3E5FA">
  <tr> 
    <td style="padding-left:15"><strong><font color="#215dc6">资源管理</font></strong> </td>
  </tr>
</table><br>
<%
...
<form name="form1" method="post" action="?action=add" onSubmit="return check_add()">
  <table width="100%" height="90" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
    <tr> 
      <td width="50" height="30" align="center">信息:</td>
      <td><input name="classid" type="text" id="classid" size="40"> 
      　排序:
        <input name="flag" type="text" id="flag" size="8"></td>
    </tr>
    <tr>
      <td height="30" align="center">图片:</td>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="24%"><input name="image" type="text" id="imagebig" size="40" style="height:20"></td>
          <td width="76%" style="padding-left:8px;"><iframe src="tongyongsc.asp" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe></td>
        </tr>
      </table></td>
    </tr>
    <tr>
...
<% 
	if trim(request.form("add"))="add" then
		...
		next
		set rs=server.createobject("adodb.recordset")
		sql="select * from jiedai_ziyuan where id="&id
		rs.open sql,conn,1,3
		rs("classid")=classid
		rs("image")=request("image")
		rs("body")=scontent
      ...
      <td valign="top">
          ...
          <table width="100%" height="60" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr bordercolor="#cccccc">
              <td width="4%" height="30" align="center">信息:</td>
              <td width="96%"><input name="classid" type="text" id="classid" value="<%=rs("classid")%>" size="40">
                排序:
            ...
            <tr bordercolor="#cccccc">
              ...
              <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td width="24%"><input name="image" type="text" id="imagebig" style="height:20" value="<%=rs("image")%>" size="40"></td>
                    <td width="76%" style="padding-left:8px;"><iframe src="tongyongsc.asp" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe></td>
                  </tr>
              </table></td>
            </tr>
