...
ElseIf Request("Del")="更新时间" Then
   set rs=conn.execute("update job_en set times = now() where ID In(" & ID & ")")
   Response.Write("<script>alert('操作成功!');location='?ss=list'</script>")
ElseIf Request("Del")="排序" Then
For i = 0 To UBound(idTmp)
  ' conn.execute("update job_en set list ="& listtTmp(i) &" where ID In(" & ID & ")")
    conn.execute("update job_en set listnum=" & listtTmp(i) & " where id=" & idTmp(i))
Next
 Response.Write("<script>alert('操作成功!');location='?ss=list'</script>")
...
ElseIf Request("Del")="删除" Then
	'set rs=conn.execute("delete from news where ID In(" & ID & ")")
			for i=1 to request("ID").count
				...
				'删除文章
				Conn.Execute("Delete from [job_en] where ID = "&newsID&"")
...
ID=request("ID")
Conn.Execute("Delete from [job_en] where ID = "&ID&"")
response.write "<script>alert('删除成功!');location='?ss=list'</script>"
end if
...
page=request("page")
set rs=server.createobject("adodb.recordset")	
sql="select * from  job_en  where id="&id
