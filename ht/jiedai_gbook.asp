...
if rs.bof then
	...
	response.write("本站留言簿<font color=red>暂无</font>留言！")
	response.write("<br><br><br><br><br><br>")
end if
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="input" style="margin-bottom:10px">
  <tr bgcolor="#DFEFFF">
    <td class="line enfont2"><b>姓名</b>：<%= rs("name") %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;留言时间：<font style="font-size:11px"><%= rs("time") %></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
    <td align="right" bgcolor="#DFEFFF" class="line">
    
    
    <span style="display:none">
    <a href="?action=reply&id=<%= rs("id") %>">回复</a> <%if rs("check")="未通过" then%><a href="?action=check&id=<%= rs("id") %>"><b>公开</b></a><%else%><b>已公开</b><%end if%> 
    </span>
    
    
    <a href="?action=del&id=<%= rs("id") %>" onClick="return window.confirm('确定删除吗?');"><img src="images/cms-ico6.gif" width="8" height="7" border="0" align="absmiddle" title="点击删除">删除</a>&nbsp; </td>
  </tr>
  <tr>
    <td colspan="2" class="enfont2 content">
        <div><b>电话：</b><%=rs("tel")%></div>
        <div><b>传真：</b><%=rs("fax")%></div>
        <div><b>邮编：</b><%=rs("web")%></div>
      <div><b>单位：</b><%=rs("email")%></div>
<div><b>地址：</b><%=rs("address")%></div>      <div><b>留言内容：</b><%=rs("body")%></div>
