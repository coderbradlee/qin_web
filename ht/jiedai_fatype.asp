...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from jiedai_fatype where flag<"&OrderBy&" order by flag desc" 
		...
		'修改前一条记录的id 
		sql2="select flag from jiedai_fatype where id="&before_id '查询前一条记录 
		...
		sql3="select flag from jiedai_fatype where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from jiedai_fatype where flag>"&OrderBy&" order by flag asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select flag from jiedai_fatype where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select flag from jiedai_fatype where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
...
if trim(request.querystring("toutiao"))="ttyes" then
	...
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_fatype where id="&id
...
set rs=server.createobject("adodb.recordset")
if classid<>"" then
sql="select * from jiedai_fatype where anclassid="&classid&" order by flag asc"
else
sql="select * from jiedai_fatype order by flag asc"
end if
...
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
  <tr align="center">
    ...
    <td width="950" align="left" class="line"><span class="style1" style="display:none">&nbsp;<img src="../uploadfile/<%= rs("tupian") %>" width="120" height="90"><br>
    </span>
    ...
    <td width="50" align="center" class="line"><a href="?action=del&id=<%= rs("id") %>" onClick="return window.confirm('确定删除吗?');"><%if instr("85,86,87",rs("id"))=0 then%>[删除]<%end if%></a></td>
  </tr>
</table>
<br>
<% 
...
'					rs1.open "select * from jiedai_fatype where id="&id,conn,1,1
'					ras.open "select * from xbz order by anclassidorder",conn,1,1
'					if ras.eof and ras.bof then
'					response.write "请先添加栏目。"
'					response.end
