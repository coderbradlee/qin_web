...
rs("toutiao")=request.form("toutiao")
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('添加成功!');location='imgg_config.asp'</script>"
end if
%>
	...
	return true;
      ...
      <form name="form1" method="post" action="?action=add" onSubmit="return check_add()">
	   ...
	   <!-- <tr>
		  ...
		  <tr >
            <td align="right">英文版显示：</td>
            <td><input type="checkbox" name="classid"  id="classid" style="cursor:hand" value="1"></td>
          </tr>
...
if trim(request.form("submit"))="修改" then
	...
	dim rs,sql
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_banner where id="&id
	rs.open sql,conn,1,3
	rs("title")=request.form("title")
	rs("classid")=request.form("classid")
...
rs("toutiao")=request.form("toutiao")
	...
	response.write "<script>alert('修改成功!');location='imgg_config.asp'</script>"
	response.end
end if
id=trim(request.querystring("id"))
set rs=server.createobject("adodb.recordset")
        ...
        <table width="100%" height="125" border="0" cellpadding="3" cellspacing="0" bordercolor="#cccccc">
         ...
         --> 
          ...
          <tr >
      	  ...
      	  <tr >
            <td align="right">英文版显示：</td>
            <td><input type="checkbox" name="classid"  id="classid" style="cursor:hand" value="1"  <%if rs("classid")="1" then response.write"checked"%>></td>
          </tr>
...
if trim(request.querystring("action"))="del" then
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('删除成功!');location='imgg_config.asp'</script>"
end if
 %>
...
if trim(request.querystring("zhiding"))="zdyes" then
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('已取消显示!');location='imgg_config.asp?Action=list&ynpage="&page&"&keywords="&keywords&"&user_ename="&user_ename&"&cid="&cid&"'</script>"
	response.end
end if
 %>
...
if trim(request.querystring("zhiding"))="zdno" then
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('置顶显示成功!');location='imgg_config.asp?Action=list&ynpage="&page&"&keywords="&keywords&"&user_ename="&user_ename&"&cid="&cid&"'</script>"
	response.end
end if
 %>
...
set rs=server.createobject("adodb.recordset")
    if toutiao<>"" then
	sql="select * from jiedai_banner where toutiao="&toutiao
	else
	sql="select * from jiedai_banner order by id desc "
    end if
	rs.open sql,conn,1,1
	rs.pagesize=2
...
%>
                      <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                          <td ><table width="181" border="0" align="center" cellpadding="0" cellspacing="3" class="zw">
                            <tr>
                              <%
i=0
do while i<1 and not rs.eof  and rowcount>0
%>
         ...
         <tr>
           <td bgcolor="#D3E9FC"><img src="../uploadfile/<%=rs("tupian")%>" height="164" width="704" border="0"  style="padding:1px; border:1px solid #B6D7EF; "></td>
         </tr>
         <tr style="display:none">
