      ...
      <form name="form1" method="post" action="?action=add" onSubmit="return check_add()">
        <table width="100%" height="125" border="0" cellpadding="3" cellspacing="0" bordercolor="#cccccc">
          ...
          <tr>
            ...
            <td>
<% 	  
sql="select * from jiedai_xxfdclass order by flag desc"
		...
		set rs=nothing
		 %>
          ...
          <tr>
            ...
            <td><input name="author" type="text" id="author" value="合大智能" size="16" style="height:20;width:100%"></td>
          </tr>
          <tr>
...
if trim(request.form("submit"))="修改" then
	...
	dim rs,sql
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_xxfd where id="&id
      ...
      <form name="form1" method="post" action="?action=edit&id=<%= trim(request.querystring("id")) %>" onSubmit="return check_edit()">
        <table width="100%" height="125" border="0" cellpadding="3" cellspacing="0" bordercolor="#cccccc">
          ...
          <tr>
            <td align="center">信息类型：</td>
            <td><% 	  
sql="select * from jiedai_xxfdclass order by flag asc"
...
<input name="keywords" type="text" class="input" id="keywords" style="width:150px;height:21px; padding-left:5px" onFocus='this.select()' onBlur="if (this.value ==''){this.value=this.defaultValue}" onClick="if(this.value=='输入信息关键词')this.value=''" value="输入信息关键词">
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="6">
   <tr>
     <td width="19%" valign="top"><table width="100%" height="107" border="0" cellpadding="8" cellspacing="1" bgcolor="#DFEFFF">
       <tr>
         <td align="center" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="5">
           ...
           <% 	  
sql_classid="select * from jiedai_xxfdclass"
set rs_classid=conn.execute(sql_classid)  
 %>
             <% do while not rs_classid.eof %>
             <a href="?action=list&cid=<%= rs_classid("id") %>">
               <%if request.QueryString("cid")=rs_classid("id") then response.write"<font color=#ff0000><b>"%>
               <%= rs_classid("classname") %></font></a>
           <table width="100%" border="0" cellspacing="0" cellpadding="0">
               <tr>
                 <td height="7"></td>
               </tr>
     ...
     <td width="81%" valign="top"><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc" background="images/bg_title.gif">
       <tr align="center">
	...
	sql="select * from jiedai_xxfd where 1=1 "
