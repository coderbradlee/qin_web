...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from fuwu where jhpx<"&OrderBy&" order by jhpx desc" 
		...
		'修改前一条记录的id 
		sql2="select jhpx from fuwu where id="&before_id '查询前一条记录 
		...
		sql3="select jhpx from fuwu where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from fuwu where jhpx>"&OrderBy&" order by jhpx asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select jhpx from fuwu where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select jhpx from fuwu where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
            ...
            <td align="center" valign="top"><table width="73%" height="33" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" class="typecss">
                      <tr>
                        <td align="center" style="padding-left:12px;"><b>案例类型</b></td>
                      </tr>
                    </table>
		...
		set rs=server.createobject("adodb.recordset")
		sql="select * from fwclass order by anclassidorder asc"
...
set rs=server.createobject("adodb.recordset")
	...
	sql="select * from fuwu where 1=1 "
...
<% 
	if trim(request.form("add"))="add" then
		...
		set rs=server.createobject("adodb.recordset")
		sql="select * from fuwu where id="&id
		rs.open sql,conn,1,3
'		rs("classid")=classid
		rs("产品介绍")=scontent
...
　排序:
  <input name="flag" type="text" id="flag" value="<%=ras("jhpx")%>" size="8"></td>
	  ...
	  set rs=server.CreateObject("adodb.recordset")
					...
					rs1.open "select * from fuwu where id="&id,conn,1,1
					rs.open "select * from fwclass order by anclassidorder",conn,1,1
					if rs.eof and rs.bof then
					response.write "请先添加栏目。"
					response.end
