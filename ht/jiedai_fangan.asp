...
if trim(request.form("submit"))="添加" then
	...
	sql="select * from jiedai_fangan"
	rs.open sql,conn,3,2
	rs.addnew
	rs("title")=request.form("title")
	rs("cpclass")=request.form("cpclass")
        ...
        <table width="100%" height="125" border="0" cellpadding="3" cellspacing="0" bordercolor="#cccccc">
		 ...
		 <tr style="display:none">
			  ...
			  <%'dim rs1
				...
				%>
        <select name="cpclass" size="1" id="cpclass" >
          <%do while not rs.eof%>
...
set rs=conn.execute(sql)  
 %>
                  <select name="newsclass" id="newsclass" style="width:100%">
                    <% do while not rs.eof %>
                    <option value="<%= rs("id") %>" <%if rs("id")=classid then response.Write("selected='selected'")%>><%= rs("classid") %></option>
                    <%
		 rs.movenext
		loop
		rs.close
		set rs=nothing
		 %>
          ...
          <tr>
            ...
            <td><input name="author" type="text" id="author" value="志凌科技" size="16" style="height:20;width:100%"></td>
          </tr>
          <tr>
...
if trim(request.form("submit"))="修改" then
	...
	dim rs,sql
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_fangan where id="&id
...
function check_edit(){
	...
	<tr  style="display:none">
			  ...
			  <%'dim rs1
					...
					'rs1.open "select * from product where id="&id,conn,1,1
					rsc.open "select * from sh_sort order by anclassidorder",conn,1,1
        ...
        <select name="cpclass" size="1" id="cpclass" >
          <%do while not rs.eof%>
          <option value="<%=rsc("anclassid")%>"  <%if rs("cpclass")=rsc("anclassid") then%>selected<%end if%> ><%=trim(rsc("anclass"))%></option>
          <%
					rsc.movenext
					loop
					end if
...
set ras=conn.execute(sql)  
 %>
                <select name="newsclass" id="select" style="width:100%">
                  ...
                  <option value="<%= ras("id") %>" <%if ras("id")=rs("classid") then response.Write("selected='selected'")%>><%= ras("classid") %></option>
                  <%
		 ras.movenext
		loop
		ras.close
 ...
 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-bottom:7px">
  <form name="form" method="post" action="?Action=list">
    <tr> 
      <td align="left">　信息搜索：
<% 	  
sql="select * from jiedai_faclass order by flag asc"
set ras=conn.execute(sql)  
 %>
     ...
     <td width="18%" valign="top"><table width="100%" height="107" border="0" cellpadding="8" cellspacing="1" bgcolor="#DFEFFF">
       <tr>
         <td align="center" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="5">
           <tr>
             <td align="center" bgcolor="#DFEFFF" style="font-size:16px"><b>方案类型</b></td>
           </tr>
         </table>
		 	  ...
		 	  <% 	  
sql_classid="select * from jiedai_fatype"
set rs_classid=conn.execute(sql_classid)  
 %>	  
	  <% do while not rs_classid.eof %>
        <a href="?action=list&cid=<%= rs_classid("id") %>"><%if request.QueryString("cid")=rs_classid("id") then response.write"<font color=#ff0000><b>"%><%= rs_classid("classid") %></font></a><table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="7"></td>
  </tr>
</table>
	...
	sql="select * from jiedai_fangan where 1=1 "
