...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from jiedai_qita where flag<"&OrderBy&" order by flag desc" 
		...
		'修改前一条记录的id 
		sql2="select flag from jiedai_qita where id="&before_id '查询前一条记录 
		...
		sql3="select flag from jiedai_qita where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from jiedai_qita where flag>"&OrderBy&" order by flag asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select flag from jiedai_qita where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select flag from jiedai_qita where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
...
<% if trim(request.querystring("action"))="add" then
	if trim(request.form("add"))="add" then
		...
		rs("e_tupian")=request.form("e_image")
...
<form name="form1" method="post" action="?action=add" onSubmit="return check_add()">
  <table width="100%" height="92" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
    ...
    <tr> 
      <td width="130" height="30" align="right">中文标示图:</td>
      <td>
      
	
    <table width="50%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><input name="image" type="text" id="tu1" value="" size="40" style="height:20; width:200px;"></td>
                <td style="padding-left:8px"><iframe src="jiedai_up.asp?ffs=form1&id=tu1" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe></td>
              </tr>
            </table>
    ...
    　</td>
        ...
        <tr> 
      <td width="130" height="30" align="right">英文标示图:</td>
      <td>
      
	
    <table width="50%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><input name="e_image" type="text" id="tu2" value="" size="40" style="height:20; width:200px;"></td>
                <td style="padding-left:8px"><iframe src="jiedai_up.asp?ffs=form1&id=tu2" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe></td>
              </tr>
            </table>
...
<% 
	if trim(request.form("add"))="add" then
		...
		next
		set rs=server.createobject("adodb.recordset")
		sql="select * from jiedai_qita where id="&id
		rs.open sql,conn,1,3
		'rs("classid")=classid
		rs("body")=scontent
		rs("tupian")=request.form("image")
...
<form name="form1" method="post" action="?action=edit&id=<%= trim(request.querystring("id")) %>" onSubmit="return check_edit()">
  <table width="100%" height="417" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
        ...
        <input name="add" type="hidden" id="add" value="add">
	      ...
	      <tr> 
      <td width="130" height="30" align="center" style="font-size:15px; color:#FF0000"><b><%= rs("classid") %></b></td>
      <td width="863">
...
<% if Trim(Request.QueryString("noimg"))<>"yes" then %>
    ...
    <table width="50%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><input name="image" type="text" id="tu1s" value="<% =rs("tupian") %>" size="40" style="height:20; width:200px;"></td>
                <td style="padding-left:8px"><iframe src="jiedai_up.asp?ffs=form1&id=tu1s" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe></td>
              </tr>
            </table>
    ...
    <table width="50%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><input name="e_image" type="text" id="tu2s" value="<% =rs("e_tupian") %>" size="40" style="height:20; width:200px;"></td>
                <td style="padding-left:8px"><iframe src="jiedai_up.asp?ffs=form1&id=tu2s" width="270" marginwidth="0" height="25" marginheight="0" scrolling="no" frameborder="0"></iframe></td>
              </tr>
            </table>
