...
if paixu="up" then '上移 
		'首先判断是不是已经移动到最前 
		sql="select top 1 id from jiedai_xyk where flag<"&OrderBy&" order by flag desc" 
		...
		'修改前一条记录的id 
		sql2="select flag from jiedai_xyk where id="&before_id '查询前一条记录 
		...
		sql3="select flag from jiedai_xyk where ID="&at_ID '查询当前记录（要移动的记录） 
...
elseif paixu="Down" then '下移与上移道理一样 
		sql="select top 1 id from jiedai_xyk where flag>"&OrderBy&" order by flag asc" 
		set rs=server.createobject("adodb.recordset") 
		rs.open sql,conn,1,3 
		if rs.eof then 
		rs.close 
		...
		sql2="select flag from jiedai_xyk where ID="&end_id 
		set rs2=server.createobject("adodb.recordset") 
		rs2.open sql2,conn,1,3 
		...
		sql3="select flag from jiedai_xyk where ID="&at_ID 
		set rs3=server.createobject("adodb.recordset") 
		rs3.open sql3,conn,1,3 
...
if trim(request.querystring("toutiao"))="ttyes" then
	...
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_xyk where id="&id
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('推荐已取消!');location='?Action=list&page="&page&"&keywords="&keywords&"&user_ename="&user_ename&"&Caseid="&Caseid&"'</script>"
	response.end
end if
...
if trim(request.querystring("toutiao"))="ttno" then
	...
	rs.close
	set rs=nothing
	response.write "<script>alert('推荐设置成功!');location='?Action=list&page="&page&"&keywords="&keywords&"&user_ename="&user_ename&"&Caseid="&Caseid&"'</script>"
	response.end
end if
...
set rs=server.createobject("adodb.recordset")
if classid<>"" then
sql="select * from jiedai_xyk where anclassid="&classid&" order by flag asc"
else
sql="select * from jiedai_xyk order by flag asc"
end if
...
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
  <tr align="center">
    ...
    <td width="950" align="left" class="line"><span class="style1">&nbsp;<img src="../uploadfile/<%= rs("tupian") %>" width="120" height="90"><br>
    </span>
      <table width="124" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="124" align="center"><%=rs("classid")%></td>
        </tr>
      </table></td>
    <td width="132" align="center" class="line">
	...
	<%if rs("cptj")=1 then%>
                <a href="?Action=list&toutiao=ttyes&jid=<%=rs("id")%>&page=<%=page%>&keywords=<%=keywords%>&Caseid=<%=Caseid%>&Nclassid=<%=Nclassid%>"><img src="images/Ok.gif" alt="已设置推荐" width="16" height="16" border="0" /></a>
                <%else%>
                <a href="?Action=list&toutiao=ttno&jid=<%=rs("id")%>&page=<%=page%>&keywords=<%=keywords%>&Caseid=<%=Caseid%>&Nclassid=<%=Nclassid%>"><img src="images/err.gif" alt="未设置推荐" width="12" height="11" border="0" /></a>
                <%end if%>
...
<form name="form1" method="post" action="?action=add" onSubmit="return check_add()">
  <table width="100%" height="92" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
    <tr>
      <td align="right" style="padding-left:15">标题:</td>
      <td height="20" align="left"><input name="classid" type="text" id="classid" size="40"></td>
    </tr>
    <tr> 
      <td width="77" height="30" align="right">排序:</td>
      <td>
  ...
  <table width="100%" border="0" cellpadding="4" cellspacing="0" >
    <tr>
	  ...
	  set ras=server.CreateObject("adodb.recordset")
					...
					rs1.open "select * from jiedai_xyk where id="&id,conn,1,1
					ras.open "select * from xbz order by anclassidorder",conn,1,1
					if ras.eof and ras.bof then
					response.write "请先添加栏目。"
					response.end
