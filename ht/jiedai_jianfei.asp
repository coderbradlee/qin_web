...
if trim(request.form("submit"))="添加" then
	...
	sql="select * from jiedai_jianfei"
      ...
      <form name="form1" method="post" action="?action=add" onSubmit="return check_add()">
        <table width="100%" height="125" border="0" cellpadding="3" cellspacing="0" bordercolor="#cccccc">
          ...
          <tr>
            ...
            <td>
<% 	  
sql="select * from jiedai_jianfeiclass order by flag asc"
set rs=conn.execute(sql)  
 %>
                  <select name="newsclass" id="newsclass" style="width:100%">
                    <% do while not rs.eof %>
                    <option value="<%= rs("id") %>" <%if rs("id")=classid then response.Write("selected='selected'")%>><%= rs("classname") %></option>
                    <%
		 rs.movenext
		loop
		rs.close
		set rs=nothing
		 %>
          ...
          <tr>
            ...
            <td><input name="author" type="text" id="author" value="莫可设计" size="16" style="height:20;width:100%"></td>
          </tr>
          <tr>
...
alle.style.display="none";
	   ...
	   <iframe id="ewebeditor1" src="../jiedaied/jiedaied.asp?id=content&style=jiedaiedit" frameborder="0" scrolling="no" width="100%" height="350"></iframe>			</td>
          </tr>
          <tr>
...
if trim(request.form("submit"))="修改" then
	...
	dim rs,sql
	set rs=server.createobject("adodb.recordset")
	sql="select * from jiedai_jianfei where id="&id
...
set ras=conn.execute(sql)  
 %>
                <select name="newsclass" id="select" style="width:100%">
                  ...
                  <option value="<%= ras("id") %>" <%if ras("id")=rs("classid") then response.Write("selected='selected'")%>><%= ras("classname") %></option>
                  <%
		 ras.movenext
		loop
		ras.close
		set ras=nothing
		 %>
          ...
          <tr style="display:none">
            <td align="center">信息简介：</td>
            <td><textarea name="jianjie" rows="4" id="jianjie" style="width:100%"><%=rs("jianjie")%></textarea></td>
          </tr>
          <tr>
            <td align="center">信息来源：</td>
         ...
         <td align="center" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="5">
           <tr>
             <td align="center" bgcolor="#DFEFFF" style="font-size:16px"><b>新闻类型</b></td>
		 	  ...
		 	  <% 	  
sql_classid="select * from jiedai_jianfeiclass"
set rs_classid=conn.execute(sql_classid)  
 %>	  
	  <% do while not rs_classid.eof %>
        <a href="?action=list&cid=<%= rs_classid("id") %>"><%if request.QueryString("cid")=rs_classid("id") then response.write"<font color=#ff0000><b>"%><%= rs_classid("classname") %></font></a><table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="7"></td>
  </tr>
</table>
     ...
     <td width="82%" valign="top"><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc" background="images/bg_title.gif">
       <tr align="center">
         ...
         <td width="50" align="center" bgcolor="#DFEFFF">头条</td>
         <td width="50" align="center" bgcolor="#DFEFFF">置顶</td>
         <td width="50" align="center" bgcolor="#DFEFFF" style="display:none">图链</td>
	...
	sql="select * from jiedai_jianfei where 1=1 "
 ...
 %>
       <table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bordercolor="#cccccc">
         <tr>
           ...
           <td class="line">&nbsp;<font color="<%=rs("titlecolor")%>"><%= rs("title") %></font></td>
           ...
           <td width="50" align="center" class="line"><%if rs("tuijian")=1 then%>
               ...
               <%end if%></td>
           <td width="50" align="center" class="line" style="display:none">
		   ...
		   <a href="/<%=rs("tupian")%>" target="_blank"><img src="images/arrow38.gif" border="0" height="21" width="21"></a>
		   <%else%>
		   &nbsp;
		   <%end if%>
